...
def main():
    ...
    # Create reverse lookup: filename -> full_path and filename -> exercise
    filename_to_path = {}
    ...
    for record in fine_labels:
        ...
        # Make path relative to BASE_DIR ("dataset")
        if full_video_path.startswith("dataset/"):
            relative_video_path = full_video_path[len("dataset/") :]
        else:
            relative_video_path = full_video_path

        # Pick assistant answer from the most descriptive label
        if record.get("labels_descriptive"):
            assistant_answer = record["labels_descriptive"]
        elif record.get("labels"):
            labels = record["labels"]
            assistant_answer = labels[0] if isinstance(labels, list) and labels else labels
        else:
        ...
        # NEW OUTPUT FORMAT:
        # - "video": ["path"] (list)
        # - No "split" field inside each item
        output_data.append(
            {
    ...
    print(f"Exercise classes (from manifest): {len(set(filename_to_exercise.values()))}")
    print("\nSplit Distribution:")
